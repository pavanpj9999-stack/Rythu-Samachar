...
import { Search, Trash2, Edit, FileSpreadsheet, Upload, ArrowLeft, Save, Image as ImageIcon, Plus, Columns, CheckCircle, X, FileText, ChevronLeft, ChevronRight, BarChart3, RefreshCw, PlusCircle, RotateCcw, File, AlertTriangle, Eye, Download, User as UserIcon, MonitorPlay, Maximize2, Minimize2, Eraser } from 'lucide-react';
...
export const UniversalDataModule: React.FC<UniversalDataModuleProps> = ({ moduleType, title, description }) => {
  ...
  const isExcelDriven = ['AREGISTER', 'DATA_6A'].includes(moduleType);
  ...
  const isMediaColumn = (colName: string) => {
      if (!isRythuDetails) return false;
  ...
  // --- COMPONENT: FULL SCREEN UPLOADED DATA VIEW ---
  if (viewMode === 'uploaded_full') {
      // (This view is mostly for read-only / bulk audit, but if we need edit support, we can propagate it. 
      // For simplicity, sticking to existing rendering logic here but enabling edit inputs same as list view)
      return (
    ...
    <div className="flex flex-col h-[calc(100vh-8rem)] animate-in fade-in slide-in-from-right-4">
                              ...
                              {paginatedRecords.length > 0 ? paginatedRecords.map((record, index) => {
                                  ...
                                  <tr key={record.id} className={rowClass}>
                                      {currentColumns.map((col, idx) => {
                                          ...
                                          if (isRythuDetails && (col.toLowerCase().includes('photo') || col.toLowerCase().includes('picture'))) {
                                               const mediaUrl = editFormData && editingId === record.id ? editFormData[col] : record[col];
                                               // STRICT: Only Admin can upload photo (Editable if Admin, View-only if Staff)
                                               const isPhotoEditable = isAdmin;
      ...
      {activeTab === 'report' && moduleType === 'AREGISTER' && (
           <div className="flex-1 p-6 bg-gray-50 overflow-auto">
              <div className="max-w-full mx-auto bg-white rounded-xl shadow-sm border border-corp-200 overflow-hidden">
                  <div className="p-6 border-b border-corp-100 flex justify-between items-center bg-gray-50">
                      <div><h3 className="text-lg font-bold text-corp-800">A-Register Summary Report</h3><p className="text-sm text-corp-500">Consolidated Extent Details</p></div>
                      <div className="flex gap-3"><button onClick={recalculateStats} className="px-4 py-2 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 font-bold text-sm flex items-center"><RefreshCw size={16} className="mr-2"/> Recalculate</button>{isAdmin && <button onClick={saveSummary} disabled={!isSummaryEdited} className={`px-4 py-2 text-white rounded-lg font-bold text-sm flex items-center shadow-lg transition-all ${isSummaryEdited ? 'bg-agri-600 hover:bg-agri-700' : 'bg-gray-400 cursor-not-allowed'}`}><Save size={16} className="mr-2"/> Save Report</button>}</div>
                  </div>
                  <div className="overflow-x-auto"><table className="w-full text-center border-collapse"><thead><tr className="bg-gray-100 text-corp-700 text-sm uppercase tracking-wide"><th className="p-4 border border-gray-200 font-bold whitespace-nowrap min-w-[140px]">Patta Dry Extent</th><th className="p-4 border border-gray-200 font-bold whitespace-nowrap min-w-[140px]">Patta Wet Extent</th><th className="p-4 border border-gray-200 font-bold whitespace-nowrap min-w-[140px]">Inam Dry Extent</th><th className="p-4 border border-gray-200 font-bold whitespace-nowrap min-w-[140px]">Inam Wet Extent</th><th className="p-4 border border-gray-200 font-bold whitespace-nowrap min-w-[140px]">Dotted Dry Extent</th><th className="p-4 border border-gray-200 font-bold whitespace-nowrap min-w-[140px]">Dotted Wet Extent</th><th className="p-4 border border-gray-200 font-bold whitespace-nowrap min-w-[140px]">UAW Extent</th><th className="p-4 border border-gray-200 font-bold whitespace-nowrap min-w-[140px]">Poramboke Extent</th><th className="p-4 border border-gray-200 font-bold whitespace-nowrap min-w-[140px] bg-green-50 text-green-900">Total Extent</th><th className="p-4 border border-gray-200 font-bold whitespace-nowrap min-w-[140px] bg-blue-50 text-blue-800">Total Entries</th></tr></thead><tbody><tr className="bg-white h-20"><td className="p-2 border border-gray-200 align-middle">{isAdmin ? (<input type="number" className="w-full p-2 text-center font-bold text-lg border border-gray-300 rounded focus:ring-2 focus:ring-agri-500 outline-none" value={summaryStats?.pattaDry || 0} onChange={e => handleSummaryChange('pattaDry', e.target.value)} />) : <span className="font-bold text-lg">{summaryStats?.pattaDry || 0}</span>}</td><td className="p-2 border border-gray-200 align-middle">{isAdmin ? (<input type="number" className="w-full p-2 text-center font-bold text-lg border border-gray-300 rounded focus:ring-2 focus:ring-agri-500 outline-none" value={summaryStats?.pattaWet || 0} onChange={e => handleSummaryChange('pattaWet', e.target.value)} />) : <span className="font-bold text-lg">{summaryStats?.pattaWet || 0}</span>}</td><td className="p-2 border border-gray-200 align-middle">{isAdmin ? (<input type="number" className="w-full p-2 text-center font-bold text-lg border border-gray-300 rounded focus:ring-2 focus:ring-agri-500 outline-none" value={summaryStats?.inamDry || 0} onChange={e => handleSummaryChange('inamDry', e.target.value)} />) : <span className="font-bold text-lg">{summaryStats?.inamDry || 0}</span>}</td><td className="p-2 border border-gray-200 align-middle">{isAdmin ? (<input type="number" className="w-full p-2 text-center font-bold text-lg border border-gray-300 rounded focus:ring-2 focus:ring-agri-500 outline-none" value={summaryStats?.inamWet || 0} onChange={e => handleSummaryChange('inamWet', e.target.value)} />) : <span className="font-bold text-lg">{summaryStats?.inamWet || 0}</span>}</td><td className="p-2 border border-gray-200 align-middle">{isAdmin ? (<input type="number" className="w-full p-2 text-center font-bold text-lg border border-gray-300 rounded focus:ring-2 focus:ring-agri-500 outline-none" value={summaryStats?.dottedDry || 0} onChange={e => handleSummaryChange('dottedDry', e.target.value)} />) : <span className="font-bold text-lg">{summaryStats?.dottedDry || 0}</span>}</td><td className="p-2 border border-gray-200 align-middle">{isAdmin ? (<input type="number" className="w-full p-2 text-center font-bold text-lg border border-gray-300 rounded focus:ring-2 focus:ring-agri-500 outline-none" value={summaryStats?.dottedWet || 0} onChange={e => handleSummaryChange('dottedWet', e.target.value)} />) : <span className="font-bold text-lg">{summaryStats?.dottedWet || 0}</span>}</td><td className="p-2 border border-gray-200 align-middle">{isAdmin ? (<input type="number" className="w-full p-2 text-center font-bold text-lg border border-gray-300 rounded focus:ring-2 focus:ring-agri-500 outline-none" value={summaryStats?.uaw || 0} onChange={e => handleSummaryChange('uaw', e.target.value)} />) : <span className="font-bold text-lg">{summaryStats?.uaw || 0}</span>}</td><td className="p-2 border border-gray-200 align-middle">{isAdmin ? (<input type="number" className="w-full p-2 text-center font-bold text-lg border border-gray-300 rounded focus:ring-2 focus:ring-agri-500 outline-none" value={summaryStats?.poramboke || 0} onChange={e => handleSummaryChange('poramboke', e.target.value)} />) : <span className="font-bold text-lg">{summaryStats?.poramboke || 0}</span>}</td><td className="p-2 border border-gray-200 align-middle bg-green-50">{isAdmin ? (<input type="number" className="w-full p-2 text-center font-bold text-xl bg-green-50 border border-green-200 rounded focus:ring-2 focus:ring-agri-500 outline-none text-green-900" value={summaryStats?.totalextent || 0} onChange={e => handleSummaryChange('totalextent', e.target.value)} />) : <span className="font-bold text-xl text-green-900">{summaryStats?.totalextent?.toFixed(2) || '0.00'}</span>}</td><td className="p-2 border border-gray-200 align-middle bg-blue-50"><span className="font-bold text-xl text-blue-900">{records.length}</span></td></tr></tbody></table></div>
              </div>
           </div>
      )}
    </div>
